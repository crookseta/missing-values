...
namespace MissingValues.Tests.Core
{
	public partial class UInt512Test
	{
		...
		{
			Assert.Equal(0x200U, BinaryIntegerHelper<UInt>.LeadingZeroCount(Zero));
			Assert.Equal(0x1FFU, BinaryIntegerHelper<UInt>.LeadingZeroCount(One));
		...
		public static void PopCountTest()
		{
			...
			Assert.Equal(0x200U, BinaryIntegerHelper<UInt>.PopCount(MaxValue));
		}
		...
		{
			Assert.Equal(0x200U, BinaryIntegerHelper<UInt>.TrailingZeroCount(Zero));
		...
		public void ParseTest()
		{
			...
			Assert.Throws<FormatException>(() => NumberBaseHelper<UInt>.Parse("13407807929942597099574024998205846127479365820592393377723561443721764030073546976801874298166903427690031858186486050853753882811946569946433649006084096", System.Globalization.NumberStyles.Integer, CultureInfo.CurrentCulture));
		}
